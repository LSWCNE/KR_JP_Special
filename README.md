...
`/`(AI 채팅)는 관리자가 `/admin`에서 "게시하기"를 누르기 전까지 기본적으로 비공개
상태입니다. 게시 전에는 일반 사용자에게 "준비 중" 안내만 보이고 `/api/ai/chat`
호출도 서버에서 403으로 거부됩니다. 관리자(로그인한 상태)는 게시 여부와 무관하게
항상 미리보기로 채팅을 사용해볼 수 있습니다.
...
app/
  ...
  services/
    ...
    sheet_sync.py             # 구글 시트 CSV 동기화 (fetch → parse → 기존 응답 전체 교체)
...
- **"지금 동기화"를 누르면 이전 응답은 전부 지워지고 그 시점의 구글 시트 내용으로 완전히
  교체됩니다.** 응답을 누적 보관하지 않고 항상 "현재 연결된 시트" 기준으로만 분석하기
  위한 설계입니다. 그만큼 국적 수동 보정·제외 처리 같은 관리자 작업도 동기화할 때마다
  함께 초기화되니, 시트를 확정한 뒤 마지막에 보정하는 것을 권장합니다.
